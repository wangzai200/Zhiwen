...
def train(model, device, train_data, test_data, args):
    ...
    train_data_loader = DataLoader(train_data, sampler=train_sampler,
                                   batch_size=train_batch_size, collate_fn=collate_func, drop_last = True)
    total_steps = int(len(train_data_loader) * args.num_train_epochs / args.gradient_accumulation_steps)
    logger.info("总训练步数为:{}".format(total_steps))
    model.to(device)

    # 添加多卡并行
    if args.device is list and len(args.device) > 1:
...
def evaluate(model, device, test_data, args):
    ...
    test_data_loader = DataLoader(test_data, sampler=test_sampler,
                                  batch_size=args.test_batch_size, collate_fn=collate_func, drop_last = True)
...
def set_args():
    ...
    parser.add_argument('--test_file_path', default='./data_dir/test.json', type=str, help='新闻标题生成的测试数据')
    ...
    parser.add_argument('--learning_rate', default=4e-4, type=float, help='模型训练时的学习率')
    parser.add_argument('--warmup_proportion', default=0.2, type=float, help='warm up概率，即训练总步长的百分之多少，进行warm up')
    ...
    parser.add_argument('--eval_steps', default=6000, type=int, help='训练时，多少步进行一次测试')
...
def main():
    ...
    # 获取device信息，用于模型训练
    device = torch.device("cuda" if torch.cuda.is_available() and args.device is not None else "cpu")
    print("device:", device)
    # 设置随机种子，方便模型复现
    if args.seed:
